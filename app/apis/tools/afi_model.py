...
def load_disease_data(filepath: str) -> tuple[dict[str, dict[str, float]], list[str]]:
...
def print_topk_and_probs(stage, disease_probs, k=3):
    """Prints top-k and all probabilities."""
    print(f"\n--- {stage} ---")
    print("Full ranked probabilities:")
    for i, (d, p) in enumerate(disease_probs, 1):
        print(f"{i:2d}. {d:<25} {p*100:6.2f}%")
    print("\nTop 3 diagnoses:")
    for i in range(min(3, len(disease_probs))):
        d, p = disease_probs[i]
        print(f"Top {i+1}: {d} ({p*100:.1f}%)")
