...
with st.form('patient_info_form'):
    ...
    date = columns[0].date_input('Date', key='date', value=datetime.today().isoformat(),
                                 width='stretch')
    patient_id: str = columns[1].text_input(
        'Patient ID', key='patient_id', width='stretch')
    patient_name: str = columns[2].text_input(
        'Patient Name', key='patient_name', width='stretch')
    patient_age: int = columns[0].number_input('Age', min_value=0, max_value=120,
                                               key='patient_age', width='stretch')
    patient_sex: str = columns[1].selectbox('Sex', options=['Male', 'Female'],
                                            key='patient_sex', index=0, width='stretch')
    race: str = columns[2].selectbox('Race', options=['White', 'Black', 'Asian', 'Hispanic', 'Other'],
